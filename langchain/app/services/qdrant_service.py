...
class QdrantService:
    ...
    def _scope_filter(
        self, subject_id: str, document_ids: Optional[list[str]] = None
    ) -> Filter:
    ...
    def search_similar(
        ...
        results = self._client.search(
            ...
            query_filter=self._scope_filter(subject_id, document_ids),
            with_payload=True,
        )
        return [
            {
    ...
    def get_random_chunks(
        ...
        results, _ = self._client.scroll(
            self._collection,
            scroll_filter=self._scope_filter(subject_id, document_ids),
            limit=200,
            with_payload=True,
            with_vectors=False,
        )
